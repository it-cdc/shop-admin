  ...
  <div>
    <el-form ref="form" :model="form" label-width="80px">
      <el-form-item label="所属类别">
        <el-cascader :options="data" :show-all-levels="false" @change="handleChange" class="ids"></el-cascader>
        当前分类的id为：{{form.category_id}}
      </el-form-item>

      <el-form-item label="排列数字">
    ...
    this.$axios({
      ...
      function loop(arr, item) {
        arr.forEach(v => {
          ...
          // 如果相等的话就吧item加入到当前分类下children
          if (v.category_id == item.parent_id) {
            // 判读当前的分类是否有children属性
            if (!v.children) {
              // 没有的话初始化为空数组
              v.children = [];
            }
